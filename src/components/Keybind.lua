...
--[[
    Keybind.lua
    Tracks capture triggers of designated key activations.
--]]
...
function KeybindComponent.Create(parentFrame: GuiObject, title: string, defaultKey: Enum.KeyCode, callback: (Enum.KeyCode) -> (), registerFlag: (any, any) -> ())
    ...
    Themes:AddPanel(bindContainer, 0.05)

    local bindStroke = Instance.new("UIStroke", bindContainer)
    bindStroke.Thickness = 1
    Themes:RegisterBorder(bindStroke)
    ...
    Themes:RegisterText(bindLabel, false)
    ...
    bindContainer.MouseEnter:Connect(function()
        ...
        if not isBinding then 
            TweenService:Create(bindStroke, Tween.FAST, {Color = Themes.GlowColor}):Play()
        end
    end)
    ...
    bindContainer.MouseLeave:Connect(function()
        isHovering = false
        if not isBinding then 
            TweenService:Create(bindStroke, Tween.FAST, {Color = Themes.CurrentTheme.Border}):Play()
        end
    end)
    ...
    Themes:OnGlowChanged(bindStroke, function(c)
        if isBinding or isHovering then
            bindStroke.Color = c
        end
    end)

    registerFlag(function() return currentKey.Name end, function(val)
        pcall(function()
