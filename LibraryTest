...
--[[
	Kaze UI Library v2.1 (Fixed & Optimized)
	...
	UPGRADED IN V2.1:
	- Full Preset Theme Engine (Arctic Light Theme & more added!)
	- FIXED: Background transparency slider is now 0 to 10 and fully functional.
	- Dynamic theme updates applied to all UI text, borders, and layouts in real-time.
	- FIXED: Dropped option selection and theme transitions no longer get stuck in light/dark state.
	- FIXED: Component titles now correctly render at size 14 to match layout calculations.
	- REMOVED: Hardcoded icon map. Fully utilizing external Lucide icons library.
--]]
...
function KazeUI:CreateWindow(config)
	...
	-- Configured theme checks with multi-option backups
	...
	if config.Glow then
		...
		elseif typeof(config.Glow) == "Color3" then
			KazeUI:SetGlow(config.Glow)
		end
	elseif config.GlowColor then
		KazeUI:SetGlow(config.GlowColor)
	end
	...
	KazeUI:OnThemeChanged(TopDivider, function(t)
	...
	KazeUI:OnThemeChanged(MiniIcon, function(t)
	...
	KazeUI:OnThemeChanged(VertDivider, function(t)
	...
	KazeUI:OnThemeChanged(ResizeHandle, function(t)
	...
	function Win:CreateThemeManager(tab)
		...
		-- Gather theme names
		local themeNames = {}
		for name, _ in pairs(KazeUI.Themes) do
	...
	function Win:CreateTab(arg1, arg2)
		...
		-- ignoreColorUpdate = true prevents theme engine from flattening tab-specific backgrounds
		...
		KazeUI:OnGlowChanged(leftIndicator, function(c)
